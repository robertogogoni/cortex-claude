# Cortex Phases H-K Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Implement 36 tasks across 4 phases (H-K) that bridge Cortex's JSONL/vector memory with human-readable `.md` topic files, add intelligence (auto-categorization, scoring, breakthroughs), advanced memory science (FSRS-6, RL, multi-hop), and ecosystem tools (TUI, dashboards, multi-agent mesh).

**Architecture:** Two-stage async pipeline: SessionEnd runs fast GapDetector (Haiku, <2s) writing `gap_report.json`; next SessionStart runs TopicCompiler (Sonnet via 3-tier fallback). New `MarkdownTopicAdapter` extends `BaseAdapter` for bidirectional `.md` read/write. All new modules follow existing patterns: CommonJS, JSDoc types, no external runtime deps for core modules.

**Tech Stack:** Node.js (CommonJS `.cjs`), no external deps for core (embedder uses `@xenova/transformers` already present), custom test runner (no jest/mocha — matches existing `test()` / `testAsync()` pattern), MCP SDK for tools.

**Dependencies:** Phase H needs B (MCP Sampling), F (FTS5), G (CRUD) — but H1-H3 can start immediately as they only need the existing adapter infrastructure. Later tasks (H5 TopicCompiler) need Sampling. Plan is ordered to maximize what can be built now.
...
## Phase I: Memory Intelligence (v3.2.0)

> **Prerequisite:** Phase H complete. The tasks below build on MarkdownTopicAdapter, GapDetector, and TopicCompiler.

### Task 13: TopicRouter — Auto-Categorization (I1)

**Files:**
- Create: `core/topic-router.cjs`
- Create: `tests/test-topic-router.cjs`

**Step 1: Write the failing test**

```javascript
// tests/test-topic-router.cjs — key test cases:
// - memory sim > 0.65 to "beeper" → assigned to beeper-troubleshooting.md
// - memory sim < 0.40 to ALL topics → tagged 'topicCandidate'
// - memory sim 0.40-0.65 → ambiguous result
// - no topic files → all memories are candidates
```

**Step 2:** Run test → FAIL

**Step 3:** Implement `TopicRouter` class with centroid comparison using text-based similarity (upgradeable to embedding when Embedder is available in the query path).

**Step 4:** Run test → PASS

**Step 5:** `git commit -m "feat(I1): add TopicRouter for auto-categorization via centroid comparison"`

---

### Task 14: BreakthroughDetector (I2)

**Files:**
- Create: `core/breakthrough-detector.cjs`
- Create: `tests/test-breakthrough-detector.cjs`

**Step 1: Write the failing test**

```javascript
// Key test cases:
// - same error in 3+ sessions → flagged as 'recurring-pattern'
// - solution evolution (X→Y→Z) → flagged as 'solution-evolution'
// - architectural insight in 4+ contexts → flagged
...
**Step 3:** Implement `BreakthroughDetector` that scans JSONL history for recurring topics (content hash similarity across sessions) and solution evolution chains.

**Step 4:** Run test → PASS

**Step 5:** `git commit -m "feat(I2): add BreakthroughDetector for cross-session pattern recognition"`

---

### Task 15: ImportanceScorer (I3)

**Files:**
- Create: `core/importance-scorer.cjs`
- Create: `tests/test-importance-scorer.cjs`
...
// - frequency normalization: 10 accesses → ~1.0, 1 access → ~0.1
// - recency: today → ~1.0, 90d → ~0.3, 180d → ~0.1
```

**Step 2:** Run test → FAIL

**Step 3:** Implement 5-dimension scorer (frequency, recency, breadth, difficulty, uniqueness) with configurable weights.

**Step 4:** Run test → PASS

**Step 5:** `git commit -m "feat(I3): add ImportanceScorer with 5-dimension scoring"`

---

### Task 16: Memory Decay for Topics (I4)

**Files:**
- Create: `core/topic-lifecycle.cjs`
- Create: `tests/test-topic-lifecycle.cjs`

**Step 1:** Test: age thresholds (fresh < 30d, aging 30-90d, stale 90-180d, archive > 180d), archived still searchable, access resets timestamp.

**Step 2:** Run test → FAIL

**Step 3:** Implement `TopicLifecycle` with status computation, archive/restore, access tracking.

**Step 4:** Run test → PASS

**Step 5:** `git commit -m "feat(I4): add TopicLifecycle for topic file aging and archival"`

---

### Task 17: Cross-Project Memory Sharing (I5)

**Files:**
- Modify: `adapters/markdown-topic-adapter.cjs` (add global dir scanning)
- Modify: `core/topic-compiler.cjs` (add scope routing logic)
- Modify: `tests/test-markdown-topic-adapter.cjs` (add global dir tests)

**Step 1:** Test: global dir discovery, project+global query merging, scope tag override.

**Step 2:** Run test → FAIL

**Step 3:** Extend `MarkdownTopicAdapter.discoverFiles()` to include `globalTopicDir`, add scope detection in `TopicCompiler`.

**Step 4:** Run test → PASS

**Step 5:** `git commit -m "feat(I5): add cross-project memory sharing via global topic directory"`

---

### Task 18: LADS Framework Integration (I6)

**Files:**
- Modify: `core/lads/` (add topic bridge metrics)
- Modify: `tests/test-lads.cjs`

**Step 1:** Test: topic file created → Documenting increments, gap filled → Adaptive increments, breakthrough → Learnable boosted, manual edit synced → Self-improving recorded.

**Step 2:** Run test → FAIL

**Step 3:** Extend LADS metrics collection with Phase H/I event hooks.

**Step 4:** Run test → PASS

**Step 5:** `git commit -m "feat(I6): integrate Phase H/I metrics into LADS framework"`

---

### Task 19: Topic Health Visualization (I7)

**Files:**
- Modify: `hooks/cli-renderer.cjs` or create `core/topic-health-renderer.cjs`
- Create: `tests/test-topic-health-renderer.cjs`

**Step 1:** Test: renders bar chart string, correct counts for each status, handles empty, truncates at 15 topics.

**Step 2:** Run test → FAIL

**Step 3:** Implement box-drawing health display following existing CortexRenderer patterns (no chalk/ora deps).

**Step 4:** Run test → PASS

**Step 5:** `git commit -m "feat(I7): add topic health visualization with bar charts"`
...
**Files:**
- Modify: `cortex/server.cjs` (add tool)
- Create: `tests/test-export-tool.cjs`

**Step 1:** Test: JSON format valid, markdown format has TOC, scope filtering works.

**Step 2:** Run test → FAIL

**Step 3:** Add `cortex__export` tool handler with json/markdown/jsonl output formats.

**Step 4:** Run test → PASS

**Step 5:** `git commit -m "feat(I8): add cortex__export MCP tool for knowledge export"`
...
**Files:**
- Modify: `adapters/base-adapter.cjs` (add provenance fields to MemoryRecord typedef)
- Create: `core/provenance-tracker.cjs`
- Create: `tests/test-provenance.cjs`

**Step 1:** Test: direct observations get source='direct', consolidated memories track mergedFrom, inferred-from-inferred downweighted.

**Step 2:** Run test → FAIL

**Step 3:** Implement provenance tracking with source chain and confidence decay for inference chains.

**Step 4:** Run test → PASS

**Step 5:** `git commit -m "feat(I9): add memory provenance tracking with source chains"`
...
**Files:**
- Modify: `hooks/session-start.cjs` (extend banner with bridge notifications)
- Modify: `hooks/injection-formatter.cjs`
- Modify: `tests/test-hooks.cjs`

**Step 1:** Test: [NEW], [STALE], [SYNC] notifications appear in banner, no bridge notifications when nothing happened.

**Step 2:** Run test → FAIL

**Step 3:** Extend `InjectionFormatter` to include bridge notification section in the Cortex banner.

**Step 4:** Run test → PASS

**Step 5:** `git commit -m "feat(I10): add bridge notifications to SessionStart banner"`

---

## Phase J: Advanced Memory Science (v3.3.0)

> **Prerequisite:** Phases F (types + anchors), G (CRUD + tracking), I (breakthroughs).
> These tasks require deeper infrastructure. Summary specs only — full TDD steps follow the same pattern as Phase H.

### Task 23: FSRS-6 Spaced Repetition (J1)

**Files:** Create `core/fsrs6.cjs`, `tests/test-fsrs6.cjs`
**Tests:** new memory stability, rating effects, interval growth, difficulty bounds, query integration.
**Impl:** 19 pre-trained weights, `computeStability()`, `nextReviewDate()`, `recordReview()`.
**Commit:** `feat(J1): add FSRS-6 spaced repetition recall engine`

### Task 24: RL-Trained Memory Manager (J2)

**Files:** Create `core/rl-trainer.cjs`, `tests/test-rl-trainer.cjs`
**Tests:** experience collection, CREATE/UPDATE/DELETE/NOOP decisions, fallback to heuristic.
**Impl:** Few-shot Haiku policy, state-action-reward tuples, training from episodic history.
**Commit:** `feat(J2): add RL-trained memory manager with Haiku policy`

### Task 25: Multi-Hop Reasoning (J3)

**Files:** Create `core/multi-hop.cjs`, `tests/test-multi-hop.cjs`
**Tests:** simple 1-hop, complex 2-3 hops, max 3 enforced, no results stops early.
**Impl:** Iterative sub-query decomposition via Haiku sufficiency check.
**Commit:** `feat(J3): add multi-hop reasoning for complex queries`

### Task 26: Harmonic Memory Representation (J4)

**Files:** Create `core/harmonic-memory.cjs`, `tests/test-harmonic-memory.cjs`
**Tests:** all sources agree, missing KG fallback, context anchor detection.
**Impl:** 3-view scoring (0.4*vector + 0.4*KG + 0.2*anchor) with graceful degradation.
**Commit:** `feat(J4): add harmonic memory with 3-view scoring`

### Task 27: Auto-Skill Extraction (J5)

**Files:** Create `core/skill-extractor.cjs`, `tests/test-skill-extractor.cjs`
**Tests:** valid YAML generation, quality gate, minimum memory threshold.
**Impl:** Triggered by BreakthroughDetector, Sonnet synthesis, quality validation, publish to skills dir.
**Commit:** `feat(J5): add auto-skill extraction via BreakthroughDetector pipeline`

### Task 28: Evolvable Memory Skills (J6)

**Files:** Create `core/evolvable-skills.cjs`, `tests/test-evolvable-skills.cjs`
**Tests:** low precision triggers threshold increase, monotonic improvement, bounded thresholds.
**Impl:** Controller-Executor-Designer loop, Haiku-generated threshold evolution.
**Commit:** `feat(J6): add evolvable consolidation with controller-executor-designer loop`

### Task 29: Cross-Session Belief Tracking (J7)

**Files:** Create `core/belief-tracker.cjs`, `tests/test-belief-tracker.cjs`
**Tests:** evolving beliefs converge, EMA decay, query returns belief+history.
**Impl:** Extends G4's `updateBelief()` with query integration for belief evolution display.
**Commit:** `feat(J7): add cross-session belief tracking with EMA evolution`

---

## Phase K: Ecosystem & Platform (v3.4.0)

> **Prerequisite:** Phase D (plugin packaging), F (types), MCP SDK features.
> These are advanced ecosystem tools — some have external dependencies (Redis, blessed, SvelteKit).

### Task 30: Hierarchical Memory Decoupling (K1)

**Files:** Create `core/hierarchical-memory.cjs`, `tests/test-hierarchical-memory.cjs`
**Tests:** theme selection, token budget enforcement, 28%+ reduction.
**Impl:** Theme → Cluster → Item hierarchy with token-aware progressive loading.
**Commit:** `feat(K1): add hierarchical memory with token-aware retrieval`

### Task 31: 4-Network Memory Separation (K2)

**Files:** Create `core/memory-networks.cjs`, `tests/test-memory-networks.cjs`
**Tests:** fact/experience/summary/belief classification, intent routing, network isolation.
**Impl:** Separate JSONL stores per network, intent-aware query routing.
**Commit:** `feat(K2): add 4-network memory separation (facts, experiences, summaries, beliefs)`

### Task 32: TUI Memory Browser (K3)

**Files:** Create `bin/cortex-tui.cjs`, `tests/test-tui.cjs`
**Deps:** `blessed` (terminal UI library — install via `npm install blessed`)
**Tests:** search mode, navigation, detail panel, performance with 1000+ entries.
**Commit:** `feat(K3): add TUI memory browser with search and navigation`

### Task 33: 3D Memory Dashboard (K4)

**Files:** Create `dashboard/` directory with SvelteKit app
**Deps:** `svelte`, `three.js` (separate package.json in `dashboard/`)
**Tests:** API endpoint returns valid graph JSON, frontend renders (playwright).
**Commit:** `feat(K4): add 3D memory dashboard with force-directed graph`

### Task 34: Streamable HTTP Transport (K5)

**Files:** Modify `cortex/server.cjs`
**Deps:** MCP SDK 2025-11-25+ with streamable HTTP
**Tests:** chunked transfer, progressive results.
**Commit:** `feat(K5): add streamable HTTP transport for progressive results`

### Task 35: MCP Tasks for Async Operations (K6)

**Files:** Modify `cortex/server.cjs`
**Deps:** MCP SDK 2025-11-25+ with task support
**Tests:** async task submission, status polling, result retrieval.
**Commit:** `feat(K6): add MCP Tasks support for long-running consolidation`

### Task 36: Multi-Agent Memory Mesh (K7)

**Files:** Create `core/memory-mesh.cjs`, `tests/test-memory-mesh.cjs`
**Deps:** `redis` (optional, graceful degradation without it)
**Tests:** pub/sub, self-message filtering, role-based filtering, reconnection.
**Commit:** `feat(K7): add multi-agent memory mesh with Redis pub/sub`

### Task 37: Git-Based Team Memory Sync (K8)

**Files:** Create `core/team-sync.cjs`, `tests/test-team-sync.cjs`
**Tests:** push/pull JSONL, merge conflict free, post-pull dedup.
**Impl:** Append-only JSONL in git repo, post-pull consolidation.
**Commit:** `feat(K8): add git-based team memory sync (append-only JSONL)`

---

## Execution Notes

### Running All Phase H Tests

```bash
# Individual module tests
node tests/test-markdown-topic-adapter.cjs
node tests/test-memory-index-manager.cjs
node tests/test-gap-detector.cjs
node tests/test-topic-compiler.cjs
node tests/test-bidirectional-sync.cjs
node tests/test-migration-bootstrap.cjs
node tests/test-audit-tool.cjs
node tests/test-topic-bridge-integration.cjs

# Full test suite (all existing + new)
for f in tests/test-*.cjs tests/*.test.cjs; do echo "=== $f ==="; node "$f"; done
```

### Key Decisions

1. **No external test framework** — matches existing Cortex test pattern using custom `test()` / `testAsync()` functions with Node.js `assert`.

2. **CommonJS (.cjs)** — matches entire codebase. No ESM conversion needed.

3. **Text-based similarity first** — The MarkdownTopicAdapter query uses word overlap scoring initially. When the Embedder singleton is available in the query path (after adapter registry wiring), it can be upgraded to semantic similarity with minimal code change.

4. **Template-first compilation** — TopicCompiler's template fallback works without any LLM. This means Phase H can ship fully functional before Phase B (MCP Sampling) is complete. When B lands, Tier 1/2 activate automatically.

5. **Feature flag** — `topicBridge.enabled = false` disables everything. Safe instant rollback.

---

*Plan saved: 2026-03-02*
*Design reference: `docs/plans/2026-03-02-cortex-unified-roadmap-phases-h-k.md`*
